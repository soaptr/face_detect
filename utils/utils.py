...
def plot_preds(img, filename, preds, config, border=3):
    #img = numpy_img.copy()
    height, width = img.shape[:2]
    for i, box in enumerate(preds):
        box = box.astype(int)
        face = img[box[1]:box[3], box[0]:box[2]]
        cv2.imwrite(config['UPLOAD_FOLDER']+str(i)+'-'+filename, face)
    for box in preds:
        box = box.astype(int)
...
def find_faces(model, img, filename, config, image_size=IMAGE_SIZE, conf_thresh=0.5, transforms=test_transform):
    model.eval()
    height, width = img.shape[:2]
    ...
    img_with_boxes = plot_preds(img, filename, boxes, config)
